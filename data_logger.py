...
class DataLogger:
    ...
    def log_session(self, current_user, login_time, wash_status, mask_status, hat_status, all_steps, wash_duration):
        ...
        try:
            ...
            logging.info(f"[LOG] Saved visit for {current_user} (Duration: {int(wash_duration)}s)")
            return True
    ...
    @staticmethod
    def send_bot_notification(current_user, login_time, wash_status, mask_status, hat_status, all_steps, wash_duration):
        try:
            bot_message = (
                f"🏥 *Smart PPE Alert*\n"
                f"👤 User: {current_user}\n"
